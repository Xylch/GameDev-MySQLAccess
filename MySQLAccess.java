...
public class MySQLAccess
{
	...
	private String host = "jdbc:mysql://host.name.here";
	private String user = "user_name_here";
	private String pass = "password_here";
	private String database = "db_name_here"; // Name of DB to use
	...
	// Returns UserID, Team #, Latitude, and Longitude of player (Hashtable<Integer, ArrayList<Double>>)
	public Hashtable<Integer, ArrayList<Double>> getPlayerLocations()
	{
		Hashtable<Integer, ArrayList<Double>> locations = new Hashtable<Integer, ArrayList<Double>>();
		ArrayList<Double> locationData;
		
		try
		{
			resultSet = statement.executeQuery("SELECT User_ID, Team, Latitude, Longitude FROM User");
			while (resultSet.next())
			{
				locationData = new ArrayList<Double>();
				// Team #
				locationData.add((double)resultSet.getInt(2));
				// Latitude (stored as string, must be converted to double)
				locationData.add(Double.parseDouble(resultSet.getString(3)));
				// Longitude (stored as string, must be converted to double)
				locationData.add(Double.parseDouble(resultSet.getString(4)));
				// Add User_ID and locationData ArrayList to Hashtable
				locations.put(resultSet.getInt(1), locationData);
			}
		}
		catch (Exception e)
		{
			System.out.println("getPlayerLocations() error: " + e.getMessage());
		}
	...
	{
		String sql = String.format("UPDATE User SET Longitude = '%s' WHERE User_ID='%d'", Double.toString(longitude), userID);
		setValue(sql);
	}
	...
	{
		String sql = String.format("UPDATE User SET Latitude = '%s' WHERE User_ID='%d'", Double.toString(latitude), userID);
		setValue(sql);
	}
